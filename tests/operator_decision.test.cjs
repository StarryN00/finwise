...
function option(id='confirm_invoice_amount') {return {id,label:'契约主操作',fields:id==='confirm_invoice_amount'?[{name:'reason',component:'textarea',label:'契约字段',required:true,placeholder:'契约占位',max_length:321}]:[],requires:['契约前置'],completion:'契约完成条件',effects:['契约影响'],not_effects:['契约边界'],available:true,unavailable_reason:''};}
...
test('descriptor text, field constraints and option drive card independently of kind',()=>{
 ...
 a.t.kind='VERIFY';assert.equal(a.render(),html);a.c.decisionNext(a.t);for(const text of ['契约完成条件','契约影响','契约边界','确认提交摘要'])assert.ok(a.render().includes(text));a.t.descriptor.options[0].label='更改后的按钮';assert.match(a.render(),/更改后的按钮/);
});
...
test('v2 automatic and personal cards are separate, unselected, bound and never prefill fields',async()=>{
 ...
 const token=a.c.materialGuidanceToken(a.t,'auto');assert.equal((a.render().match(/data-guide="suggestion-select"/g)||[]).length,0);a.c.materialGuidanceChoose(a.t,'auto',0,token);assert.match(a.render(),/is-selected/);assert.match(a.render(),/提交处理建议，继续核对/);assert.doesNotMatch(a.render(),/invoiceAmountForm/);assert.equal(a.draft.note,'原草稿');assert.equal(a.c.decisionCanSubmit(a.t),false);await a.c.materialGuidanceSubmit(a.t,'auto',0,token);assert.match(a.render(),/invoiceAmountForm/);
...
test('free scheme sends only explicit user_text; same-text retry idempotent, save opinion never requests model',async()=>{
 const a=app();a.t.material_opinions=[];a.t.descriptor.fingerprint='fp';const calls=[];a.c.request=async(url,body)=>{calls.push([url,body]);throw Error('offline');};a.c.command=async(...args)=>{calls.push(args);return {effect:{object:{object_id:'opinion',status:'SAVED_NOT_EXECUTED'}}};};a.c.loadWorkbench=async()=>{};
...
test('unmapped suggestion can be explicitly submitted as an opinion only',async()=>{
 const a=app();a.t.descriptor.fingerprint='fp';a.t.material_opinions=[];a.t.material_guidance={status:'NEEDS_HUMAN',descriptor_hash:'fp',candidates:[{option_id:null,reason:'建议将流水按原交易日期归属二月',uncertainties:['目标期间尚待确认'],prefill:{},confidence:.6,evidence_refs:['record-1'],steps:[]}]};const calls=[];a.c.command=async(...args)=>{calls.push(args);return {effect:{object:{status:'SAVED_NOT_EXECUTED'}}};};a.c.loadWorkbench=async()=>{};const token=a.c.materialGuidanceToken(a.t,'auto');a.c.materialGuidanceChoose(a.t,'auto',0,token);assert.match(a.c.renderMaterialGuidanceChannel(a.t,'auto'),/提交处理建议（仅记录）/);await a.c.materialGuidanceSubmit(a.t,'auto',0,token);assert.equal(calls.length,1);assert.equal(calls[0][0],'save_material_opinion');assert.match(calls[0][3].reason,/确认业务期间归属/);assert.match(calls[0][3].reason,/尚未映射为可执行操作/);assert.equal(a.c.decisionGuide(a.t).suggestionChoice.submitted,true);assert.equal(a.c.decisionGuide(a.t).option,'confirm_invoice_amount');
 const html=a.render();assert.match(html,/处理建议已记录/);assert.match(html,/返回问题列表/);assert.match(html,/本次只保存了处理意见，没有执行财务处理/);assert.doesNotMatch(html,/请选择处理方式/);assert.doesNotMatch(html,/data-guide="suggestion-submit"/);
...
test('server-recorded opinion restores selected status after refresh without reopening selection',()=>{
 const a=app();a.t.descriptor.fingerprint='fp';a.t.material_opinions=[{valid:true,status:'SAVED_NOT_EXECUTED',text:'按原交易日期归属二月'}];const html=a.render();
 assert.match(html,/处理方式已选择/);assert.match(html,/按原交易日期归属二月/);assert.doesNotMatch(html,/请选择处理方式/);assert.doesNotMatch(html,/data-guide="option"/);
});

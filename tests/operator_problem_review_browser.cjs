...
(async()=>{
 ...
 try{
  ...
  await page.addScriptTag({content:`
   ...
   const option={id:'confirm_invoice_amount',label:'确认已核对',available:true,unavailable_reason:'',requires:[],effects:['仅解除金额提醒'],not_effects:['不放行账务'],completion:'核对金额和业务原因',fields:[{name:'reason',label:'核对说明',component:'textarea',required:true,max_length:2000,placeholder:''}],steps:[{id:'reason',prompt:'请填写核对说明',fields:['reason']}]};
   const binding={...fixtureScope,artifact:{id:'a',version:1,sha256:'hash'},records:[{id:'r',version:1,source_anchor:record.source_anchor}]};
   const task={id:'task',kind:'INVOICE_AMOUNT',filename:'合成发票.xlsx',artifact_id:'a',artifact_version:1,record_ids:['r'],problem_review:job,descriptor:{version:'decision-v1',title:'发票金额核对',scope:binding,why:{facts:'合成原件金额',rule:'红字金额',recommendation:'核对原件',origin:'RULE',evidence:binding.records},options:[option],steps:[]}};
  ...
  fail=false;await page.evaluate(()=>{state.role='operator';materialState().screen='list';render();});await page.locator('[data-action="problem-review-request"]').click();await page.getByRole('status').filter({hasText:'复核请求已接收'}).waitFor();assert.equal(writes[1].action,'request_problem_review');assert.equal(writes[1].id,'period');assert.equal(writes[1].version,3);
  assert.equal(await page.locator('[data-action="problem-review-request"]').count(),0);assert.match(await page.locator('.problem-review').innerText(),/排队或处理中/);

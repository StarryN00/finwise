...
(async()=>{
 ...
 try{
  ...
  await page.locator('.mat-issue-group').filter({hasText:'票据业务与科目待确认'}).getByRole('button',{name:'进入处理'}).click();
  ...
  await check('independent bill form has no preconfirmed period or account and preserves source',async()=>{
   assert.equal(await page.locator('#billPeriod').count(),0);assert.equal(await page.locator('#billAccountName').count(),0);
  ...
  await check('failed confirmation retains text; save without attachment advances exactly one bill',async()=>{
   ...
   const before=commands.length;await g.review();assert.match(await page.locator('.decision-summary').innerText(),/确认执行内容/);assert.equal(commands.length,before);assert.match(await page.locator('.decision-answer').allTextContents().then(x=>x.join(' ')),/应收票据/);
   ...
   await g.commit();assert.match(await page.locator('.decision-key-table').innerText(),/3,4/);
  ...
  await check('history holding can select enterprise candidate and uses month precision',async()=>{
   await fill('HELD',true);await g.finish();assert.match(await page.locator('.decision-key-table').innerText(),/5,6/);
  ...
  await check('defer ends group with separate human and pending totals',async()=>{
   ...
   assert.match(await page.locator('.mat-group-totals').innerText(),/人工补充确认/);assert.match(await page.locator('.mat-group-totals').innerText(),/等待外部资料或条件/);
  ...
  await check('deferred bill leaves the user queue and is shown as waiting for external conditions',async()=>{
   ...
   await page.locator('.mat-issue-group').filter({hasText:'票据业务与科目待确认'}).getByRole('button',{name:'查看状态'}).click();
   assert.match(await page.locator('.decision-waiting').innerText(),/你当前无需操作/);
   assert.equal(await page.locator('#billForm').count(),0);assert.equal(await page.evaluate(()=>state.overview.material_review.flow.counts.waiting_external),1);
  });

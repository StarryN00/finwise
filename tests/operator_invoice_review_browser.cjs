...
(async()=>{
 ...
 try{
  ...
  const open=()=>page.locator('.mat-issue-group').filter({hasText:'红字或零金额'}).getByRole('button',{name:/进入处理|查看处理记录/}).click();
  await open();
  ...
  await check('failed save keeps draft; no attachment save advances one invoice',async()=>{
   ...
   await page.locator('#materialNote').press('Enter');assert.equal(commands.length,0);await g.review();assert.match(await page.locator('.decision-summary').innerText(),/确认提交摘要/);assert.equal(commands.length,0);
  ...
  await check('zero amount confirmation; defer and group summary keep distinct outcomes',async()=>{
   await page.locator('#materialNote').fill('原始零金额已核对，业务原因已确认。');await g.finish();
   ...
   assert.match(await page.locator('.mat-group-totals').innerText(),/金额核对已处理/);assert.match(await page.locator('.mat-group-totals').innerText(),/已记录待补/);
  });
  ...
  await check('deferred invoice can resume; period problem is not cleared by amount review',async()=>{
   await page.locator('[data-filter="all"]').first().click();await page.locator('[data-filter="deferred"]').click();await open();await click('invoice-amount-resume');
   await g.choose('confirm_invoice_amount');await page.locator('#materialNote').fill('已确认是历史冲红，期间问题另行核对。');await g.finish();

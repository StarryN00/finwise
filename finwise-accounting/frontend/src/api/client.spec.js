...
describe('api client', () => {
  it('uses extended timeouts for AI operations that can exceed the default request budget', () => {
    ...
    expect(source).toContain('createPreprocessJob: (packageId) => apiClient.post(`/monthly-packages/${packageId}/voucher-preprocess-jobs`)')
    expect(source).toContain('latestPreprocessJob: (packageId) => apiClient.get(`/monthly-packages/${packageId}/voucher-preprocess-jobs/latest`)')
    expect(source).toContain('getPreprocessJob: (jobId) => apiClient.get(`/voucher-preprocess-jobs/${jobId}`)')
    expect(source).toContain('cancelPreprocessJob: (jobId) => apiClient.post(`/voucher-preprocess-jobs/${jobId}/cancel`)')

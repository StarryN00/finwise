...
function decisionField(t,o,f){
  if(f.component==='textarea'){const id=decisionAdapters[o.id].fields[f.name],value=materialDraft(t)[id==='materialReason'?'reason':'note'];return `<label for="${id}">${esc(f.label)}<textarea id="${id}" ${!o.available||!materialCanWrite()?'disabled':''} ${f.required?'required':''} maxlength="${f.max_length}" placeholder="${esc(f.placeholder)}">${esc(value||'')}</textarea></label>`;}
  if(f.slot==='record_selection')return `<div>${esc(f.label)}${decisionSource(t,o.available&&materialCanWrite())}</div>`;
  if(f.slot==='period_record_selection')return typeof bankPeriodFields==='function'?bankPeriodFields(t,o.available&&decisionReady(t)):'<p>跨期办理组件尚未加载，本项未执行。</p>';
...
function decisionChoose(t,id,preserveSuggestion=false){
  ...
  const o=t.descriptor.options.find(o=>o.id===id);if(!o?.available)return;
...
function decisionNext(t,form){
  ...
  if(!o.available||g.step>=steps.length)return;
...
function decisionCanSubmit(t){
  ...
  const o=decisionOption(t),g=decisionGuide(t),steps=decisionSteps(t,o);
  return (!decisionRequiresChoice(t)||g.chosen===true)&&steps.length>0&&o.available&&g.step===steps.length&&g.reviewed===decisionAnswerHash(t,o)&&steps.every((s,i)=>decisionValidateStep(t,o,i));
}
...
function decisionCommit(t,form){
  ...
  try{
    ...
    if(a.submit){decisionAllowedSubmits.add(form);form.requestSubmit();}
...
function decisionSuggestionCards(t,candidates){
  ...
  const cards=(Array.isArray(candidates)?candidates:[]).filter(s=>{
    ...
    if(!t.descriptor.options.some(o=>o.id===s.option_id))return false;seen.add(s.option_id);return true;
  }).slice(0,3);
  ...
  return '<section class="decision-suggestions" aria-label="处理建议"><p class="small muted">以下是系统整理的处理方式。请点击一张卡片选择，选中后再继续核对；系统不会自动执行。</p>'+cards.map(s=>{
    ...
    return '<article class="decision-suggestion'+(chosen?' is-selected':'')+'" data-guide="legacy-suggestion-card" data-option="'+esc(o.id)+'" tabindex="0" role="group" aria-label="'+esc(o.label)+'" aria-selected="'+chosen+'"><div class="suggestion-heading"><h4>'+esc(o.label)+'</h4><span class="suggestion-state">'+(chosen?'已选中，待提交':'点击卡片选择')+'</span></div><p>'+esc(s.reason)+'</p><ul>'+s.uncertainties.map(x=>'<li>还需要确认：'+esc(x)+'</li>').join('')+'</ul><p><strong>选择后：</strong>'+o.effects.map(esc).join('；')+'</p><p class="small muted"><strong>不会改变：</strong>'+o.not_effects.map(esc).join('；')+'</p>'+(!o.available?'<p>'+esc(o.unavailable_reason)+'</p>':'')+'</article>';
  }).join('')+'</section>';
}
function decisionUseSuggestion(t,id){
  if(!decisionReady(t))return;
  const g=decisionGuide(t),r=g.agent?.response;
  if(!decisionAgentValid(r,t.descriptor.fingerprint)||r.status!=='PROPOSED'||r.suggestion?.option_id!==id||!t.descriptor.options.some(o=>o.id===id&&o.available))return;
...
function decisionAgentPanel(t){
  ...
  const agent=decisionGuide(t).agent,r=agent?.response,s=r?.suggestion,available=t.descriptor.options.filter(o=>o.available),proposed=available.find(o=>o.id===s?.option_id);
...
function renderDecisionTask(t){
  ...
  const summary=steps.length>0&&g.step===steps.length,paused=t.deferred&&!draft.resume&&!draft.defer,disabled=!materialCanWrite()||!o.available||paused,presentation=decisionPresentation(t);
  const list=(title,items)=>items.length?'<details><summary>'+title+'</summary><ul>'+items.map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul></details>':'';

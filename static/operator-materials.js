...
function materialDraft(t){const ui=materialState();if(!ui.drafts.has(t.id)){const saved=materialStoredDraft(ui.key,t.id),same=typeof materialTaskFingerprint==='function'&&saved?.fingerprint===materialTaskFingerprint(t),ids=new Set(t.record_ids||[]);ui.drafts.set(t.id,{note:typeof saved?.note==='string'?saved.note:'',reason:typeof saved?.reason==='string'?saved.reason:'',taskAction:saved?.taskAction&&typeof saved.taskAction==='object'?saved.taskAction:{},selected:new Set(same&&Array.isArray(saved?.selected)?saved.selected.filter(id=>ids.has(id)):[]),defer:saved?.defer===true,resume:saved?.resume===true,bill:saved?.bill&&typeof saved.bill==='object'?saved.bill:null});}return ui.drafts.get(t.id);}
...
const materialFieldLabels={employer_housing_fund:'单位公积金',employee_housing_fund:'个人公积金',invoice_status:'发票状态',payment_total:'付款金额',transaction_date:'交易日期',transaction_id:'交易流水号',balance:'账户余额',bank_account_ref:'本方账户',invoice_no:'发票号码',date:'业务日期',invoice_date:'开票日期',tax_amount:'税额',invoice_total:'价税合计',net_amount:'不含税金额',amount:'金额',period:'业务期间',aggregation_level:'汇总层级',insurance_type:'险种',employer_amount:'单位应缴',employee_amount:'个人应缴',total:'合计',employer_base:'单位基数',employee_base:'个人基数',people_count:'人数',payment_status:'缴费状态',actual_salary:'实发工资',basic_salary:'基本工资',person_name:'姓名',person_id:'个人编号',period_ref:'费款所属期',employee_social:'个人社保',employer_social:'单位社保',housing_fund:'公积金',acceptance_no:'票据包号',sub_range:'子票区间',issue_date:'出票日期',maturity_date:'到期日期',status:'票据状态',issuer:'出票人',payee:'收款人',endorser:'背书人',endorsee:'被背书人',register_kind:'资料性质',receipt_total:'收款金额',summary:'摘要',currency:'币种',bank_account_origin:'账户来源',start_period:'起始期间',end_period:'截止期间',seller_name:'销售方',buyer_name:'购买方',seller_tax_id:'销方识别号',buyer_tax_id:'购方识别号',counterparty:'对方名称',counterparty_account:'对方账户',note:'附言',base:'缴费基数',arrival_date:'到账日期',acceptance_type:'票据类型',transaction_type:'业务类型',account:'账号',entry_date:'入账日期',income:'收入',expense:'支出',tax:'税额'};
Object.assign(materialFieldLabels,{
  income:'收入金额',expense:'支出金额',employer_amount:'单位缴费金额',employee_amount:'个人缴费金额',
  net_pay:'实发工资',
  account:'个人公积金账号',account_code:'科目编码',account_name:'科目名称',
  opening_debit:'期初借方余额',opening_credit:'期初贷方余额',
  contract_no:'合同编号',contract_date:'合同日期',supplier:'供应商',
  stock_in_no:'入库单号',stock_in_date:'入库日期'
});
...
function renderMaterialTask(){
  ...
  if(!t){const empty={ready:['当前没有待确认执行的事项','系统形成可执行方案后，会在这里给出明确的确认动作。'],system:['当前没有系统处理中事项','系统整理、复核和检查任务会集中显示在这里。'],deferred:['当前没有等待外部的事项','暂无法确认且已记录原因的事项会显示在这里。']}[ui.filter]||[m.counts.files?'你当前无需处理其他资料事项':'接收本期资料',m.counts.files?'系统会在“现在要做”中给出下一项明确操作。':'本期尚无业务原件，请添加发票、银行与票据或薪酬税费资料。'],hasPrimary=state.overview.material_review?.next_step?.owner==='USER'&&state.overview.material_review?.next_step?.action;return `<section class="panel pad"><h3 id="materialTaskTitle" tabindex="-1">${empty[0]}</h3><p>${empty[1]}</p>${m.counts.files?`<button class="secondary" data-action="material-filter" data-filter="results">查看已有成果</button>${ui.skipped.size?'<button class="text" data-action="material-resume">继续查看暂未处理的事项</button>':''}`:hasPrimary?'':primary('upload','添加本期资料',!materialCanWrite())}</section>`;}
...
function renderActionMetrics(){
  const metrics=state.overview.material_review?.action_metrics;if(!metrics)return '';
  const ratio=(Number(metrics.fallback_ratio||0)*100).toFixed(1);
  return `<details class="panel pad action-metrics"><summary>查看动作使用与兜底占比</summary><p class="small muted">当前账套期间内成功执行的已声明任务动作；每个数字均可查看对应审计命令。</p><div class="action-metrics-list"><button class="secondary" data-action="material-action-metric" data-metric="all">已执行动作 ${metrics.executed_action_count} 次</button><button class="secondary" data-action="material-action-metric" data-metric="fallback">兜底动作 ${metrics.fallback_action_count} 次（${ratio}%）</button>${metrics.items.map(item=>`<button class="text" data-action="material-action-metric" data-action-type="${esc(item.action_type_id)}">${esc(item.label)} ${item.count} 次 →</button>`).join('')}</div></details>`;
}
function renderMaterialWorkbench(){
  ...
  ${renderActionMetrics()}
  <details class="panel pad mat-history"><summary>查看历史处理交接</summary><p>${history.done?'已保留历史处理意见。':'历史资料仍有未完成的核对事项。'}${state.overview.baseline_validation.status==='VALID'?'期初已通过原有校验。':'期初余额仍待核实，不能据此放行账务。'}</p><button class="text" data-action="historical-details">查看历史处理记录</button></details>
  <details class="panel pad mat-overview"><summary>${month}月资料概览与统计口径</summary><p class="small muted">原件、提取记录、人工任务与账务可用是不同口径，不相加。具体数量请通过下方分类按钮下钻。</p><button class="secondary" data-action="material-files">查看全部原件</button><details class="small muted mat-method"><summary>查看统计方法</summary>系统提取检查、人工原件核实、业务归属和账务门禁分别留痕；任务按原件与问题聚合，不与记录数相加。</details></details>
...
function installMaterialActions(){
  ...
  actions['material-action-metric']=b=>{const metrics=state.overview.material_review.action_metrics,rows=metrics.items.filter(item=>b.dataset.metric==='all'||b.dataset.metric==='fallback'&&item.fallback||b.dataset.actionType===item.action_type_id).flatMap(item=>item.drilldown.map(row=>({...row,label:item.label})));openDialog('动作执行记录',rows.length?`<div class="table-wrap"><table><thead><tr><th>动作</th><th>时间</th><th>目标</th><th>命令记录</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${esc(row.label)}</td><td>${esc(row.created_at)}</td><td class="mono">${esc(row.target_id)}</td><td class="mono">${esc(row.command_id)}</td></tr>`).join('')}</tbody></table></div>`:'<p>当前范围没有对应动作记录。</p>');};
  ...
  actions['material-page']=b=>{materialState().page=Math.max(0,Number(b.dataset.page)||0);materialSaveNavigation();render();$('materialTaskTitle')?.focus({preventScroll:true});};
  ...
  actions['material-verify']=()=>exclusive(async()=>{const t=currentMaterialTask();if(!t||!materialCanWrite()||!materialTaskIsCurrent(t))return;const d=materialDraft(t),all=state.overview.material_review.records.filter(r=>t.record_ids.includes(r.object_id)),records=all.filter(r=>d.selected.has(r.object_id)).map(r=>({object_id:r.object_id,version:r.version}));if(!records.length)throw Error('请先比较数据，并选择当前事项中已核对的记录。');const epoch=state.epoch,group=materialState().group,draftRevision=materialDraftRevision(d);await command('verify_source_values',t.artifact_id,t.artifact_version,{task_id:t.id,records,note:d.note});if(epoch!==state.epoch)return;const draftCleared=materialDeleteDraft(t.id,draftRevision);if(!draftCleared||!materialDetailStillOpen(group,t.id)){notify('资料核实已保存，可返回列表查看结果。');return;}materialAdvance(t,'verified');});
  ...
  document.addEventListener('input',e=>{if(!e.target.closest('#materialWorkbench'))return;const t=currentMaterialTask();if(!t)return;const d=materialDraft(t);if(e.target.id==='materialNote')d.note=e.target.value;if(e.target.id==='materialReason')d.reason=e.target.value;if(e.target.dataset.materialRecord){if(e.target.checked)d.selected.add(e.target.dataset.materialRecord);else d.selected.delete(e.target.dataset.materialRecord);if(typeof decisionRefreshSelectionStatus==='function')decisionRefreshSelectionStatus(t);}materialSaveDraft(t);});

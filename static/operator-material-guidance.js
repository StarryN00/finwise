...
async function materialGuidanceSubmit(t,channel,index,token){
  ...
  try{
    ...
    if(result?.effect?.status!=='WAITING_SYSTEM'||result?.effect?.object?.status!=='SAVED_NOT_EXECUTED'||!result?.effect?.job)throw Error('未取得系统处理记录，请重试。');
    if(!current())return;
    choice.submitted=true;choice.submitting=false;choice.message='意见已记录，系统正在整理可执行方案；你暂时无需处理。';decisionRemember(t);await loadWorkbench();
...
function renderMaterialGuidanceChannel(t,channel){
  ...
  const labels={QUEUED:'等待分析',RUNNING:'正在分析',PROPOSED:'待选择，尚未执行',NEEDS_HUMAN:'暂未形成可执行方案',LOCAL_ONLY:'意见已保存在本地',FAILED:'分析失败',STALE:'依据已变化'};
  ...
  return '<section class="decision-suggestions" aria-label="'+(channel==='auto'?'自动处理建议':'我的方案验证')+'"><h4>'+(channel==='auto'?'自动处理建议':'我的方案验证')+'</h4><p class="small muted">'+esc(r?(labels[r.status]||'建议状态待检查'):'暂无建议')+' · '+origin+(submittedChannel?'。本次建议已记录，不能重复选择。':'。请点击一张卡片选择处理方式，提交前仍需你确认，系统不会直接改动账务。')+'</p>'+(r?.message?'<p>'+esc(r.message)+'</p>':'')+cs.map((c,i)=>{
    ...
    const submit=!o&&chosen&&!submitted?'<button type="button" class="primary" data-guide="suggestion-submit" data-channel="'+channel+'" data-index="'+i+'" data-guidance-token="'+esc(token)+'">记录意见并交系统处理</button>':'';
    return '<article class="decision-suggestion'+(chosen?' is-selected':'')+'" data-guide="'+cardGuide+'" data-channel="'+channel+'" data-index="'+i+'" data-guidance-token="'+esc(token)+'"'+cardTab+' role="group" aria-label="'+esc(label)+'" aria-selected="'+chosen+'"><div class="suggestion-heading"><h4>'+esc(label)+(i===recommended?' <small class="badge">推荐</small>':'')+'</h4><span class="suggestion-state">'+(submitted?'系统正在整理':submittedChannel?'本次未选择':chosen?'待交系统处理':'点击卡片选择')+'</span></div><p class="small muted">'+(o?'点击后填写必要信息，再明确确认': '当前还不能直接执行，可把意见交给系统继续整理')+'</p><p>'+esc(plainReason)+'</p>'+(plainUncertainties.length?'<ul>'+plainUncertainties.map(s=>'<li>还需要确认：'+esc(s)+'</li>').join('')+'</ul>':'')+(o?'<p>执行后：'+esc(impact.effects)+'</p><p class="small muted">不会改变：'+esc(impact.notEffects)+'</p>':'')+'<details><summary>查看依据</summary><p>系统判断：'+esc(materialGuidancePlainText(c.reason))+'</p><p>依据位置：'+c.evidence_refs.map(ref=>ref==='task'?'当前事项':esc(t.descriptor.scope.records[Number(ref.slice(7))-1]?.source_anchor?.region||'来源定位待查')).join('；')+'</p>'+(c.steps.length?'<ol>'+c.steps.map(s=>'<li>'+esc(t.descriptor.options.find(o=>o.id===s.option_id)?.label)+'：'+esc(materialGuidancePlainText(s.instruction))+'</li>').join('')+'</ol>':'')+'</details><div class="suggestion-actions">'+submit+'</div>'+(chosen&&g.suggestionChoice?.message?'<p class="small" role="status">'+esc(g.suggestionChoice.message)+'</p>':'')+'</article>';
...
function renderMaterialGuidance(t){
  ...
  return '<section class="decision-agent" aria-label="处理建议与自由方案">'+renderMaterialGuidanceChannel(t,'auto')+(t.personal_material_guidance||custom.response?renderMaterialGuidanceChannel(t,'personal'):'')+'<label for="materialCustomText">我有其他处理方式<textarea id="materialCustomText" maxlength="2000" '+(disabled?'disabled':'')+' placeholder="简要说明你希望怎样处理；系统不会直接修改日期、金额或科目。">'+esc(custom.text||'')+'</textarea></label><p class="small muted">系统会把你的想法整理为现有处理方式。能执行时仍需你明确确认；暂不支持时转交系统处理。</p><button type="button" class="secondary" data-guide="custom-validate" '+(disabled||!t.descriptor.fingerprint?'disabled':'')+'>整理为处理方案</button><button type="button" class="text" data-guide="opinion-save" '+(disabled||!t.descriptor.fingerprint?'disabled':'')+'>记录意见并交系统处理</button><button type="button" class="text" data-guide="guidance-refresh">刷新建议状态</button>'+(custom.message?'<p role="status">'+esc(custom.message)+'</p>':'')+(Array.isArray(t.material_opinions)&&t.material_opinions.length?'<details><summary>我的已记录意见</summary>'+t.material_opinions.filter(x=>x.actor_id===state.user?.user_id).map(x=>'<p>'+esc(x.text)+' · '+(x.valid?'系统处理中':'依据已变化')+'</p>').join('')+'</details>':'')+'</section>';
}
...
async function materialGuidanceSaveOpinion(t){
  ...
  try{const result=await command('save_material_opinion',t.artifact_id,t.artifact_version,{task_id:t.id,descriptor_hash:t.descriptor.fingerprint,reason},false);if(result?.effect?.status!=='WAITING_SYSTEM'||result?.effect?.object?.status!=='SAVED_NOT_EXECUTED'||!result?.effect?.job)throw Error('未取得系统处理记录，请重试。');if(!current())return;custom.message='意见已记录，系统正在整理可执行方案；你暂时无需处理。';await loadWorkbench();}
...
function renderMaterialGuidanceOverview(){
  ...
  return '<details class="panel pad system-tools"><summary>系统处理工具</summary><p class="small muted">建议通常会在后台自动整理；打开页面和刷新不会重复调用。以下入口仅用于管理或重试。</p>'+(materialCanWrite()?'<button type="button" class="secondary" data-action="material-guidance-request" data-guidance-token="'+esc(materialGuidancePeriodToken())+'">重新整理本期处理建议</button>':'')+'</details>';
}
